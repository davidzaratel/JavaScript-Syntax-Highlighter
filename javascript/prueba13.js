...
dojox.data.tests.stores.OpenSearchStore.getAtomStore = function () {
  var store = new dojox.data.OpenSearchStore({
    url: dojo
      .moduleUrl("dojox.data.tests.stores", "opensearch_atom.xml")
      .toString(),
  });
...
dojox.data.tests.stores.OpenSearchStore.getRSSStore = function () {
  var store = new dojox.data.OpenSearchStore({
    url: dojo
      .moduleUrl("dojox.data.tests.stores", "opensearch_rss.xml")
      .toString(),
  });
  store._createSearchUrl = function (request) {
...
dojox.data.tests.stores.OpenSearchStore.getHTMLStore = function () {
  var store = new dojox.data.OpenSearchStore({
    url: dojo
      .moduleUrl("dojox.data.tests.stores", "opensearch_html.xml")
      .toString(),
    itemPath: "table tbody tr",
  });
  store._createSearchUrl = function (request) {
  ...
  },
  {
    ...
    runTest: function (t) {
      ...
      function onComplete(items, request) {
        ...
        t.assertEqual(
          dojox.xml.parser.textContent(id[0]),
  ...
  },
  {
    ...
    runTest: function (t) {
      ...
      function onComplete(items, request) {
        ...
        t.assertEqual(
          dojox.xml.parser.textContent(link[0]),
          "http://shaneosullivan.wordpress.com/2008/01/22/using-aol-hosted-dojo-with-your-custom-code/"
        );
        t.assertTrue(store.hasAttribute(item, "content"));
  ...
  },
  {
    ...
    runTest: function (t) {
      ...
      function onComplete(items, request) {
        ...
        t.assertTrue(store.hasAttribute(item, "content"));
        t.assertEqual(
          store.getValue(item, "content").length,
          dojo.isIE ? 53 : 64
        );
        d.callback(true);
      }
